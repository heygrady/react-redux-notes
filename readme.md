...
6. **[Build the Todo example](http://redux.js.org/docs/basics/Actions.html)** - The manual links to a [complete Todo app](http://redux.js.org/docs/introduction/Examples.html#todos) built to match what's documented in the manual. Because the Redux API is so transparent, much of the concepts only make sense onse oyu start to write code. This means that, unlike other frameworks wher you can read yhe API for hours, Redux can be best learned by actually writing some code.
...
*Note:* If you're trying to read about React, ignore how they use the old `React.createClass()` syntax in the manual. Never use something like `React.createClass()` unless, inexplicably, you're not allowed to use ES6. But really... you should be using ES6 no matter what by this point. There is no reason not to be on this train.
